    ...
    const emptyRow = `
      <tr>
        <td colspan="6" class="py-8 text-center text-slate-500 font-mono">
          <div class="empty-state space-y-1">
            <svg class="w-8 h-8 mx-auto text-slate-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
            <p class="text-xs font-semibold text-slate-400">No hazard detections logged yet.</p>
            <p class="text-[10px] text-slate-500">Live telemetry stream active. Awaiting hardware detections.</p>
          </div>
        </td>
      </tr>
    `;
      ...
      return dets.map(det => `
        <tr class="hover:bg-slate-800/40 cursor-pointer" data-det-id="${det.id}">
          <td class="py-2.5 px-3">
            <span class="status-badge ${det.type === 'POTHOLE' ? 'badge-danger' : 'badge-warning'}">
              ${det.type}
            </span>
          </td>
          <td class="py-2.5 px-3 font-bold ${det.severity === 'CRITICAL' ? 'text-red-400' : 'text-amber-400'}">${det.severity}</td>
          <td class="py-2.5 px-3 text-blue-400 font-bold font-mono">${det.distance} cm</td>
